...
;; Defines the usda2 Stablecoin according to the SIP-010 Standard
(define-fungible-token usda2)
...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance usda2 account))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? usda2 amount sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
